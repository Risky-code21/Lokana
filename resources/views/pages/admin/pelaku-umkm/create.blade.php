...
@section('title', 'Tambah Pelaku UMKM')
@section('page-title', 'Tambah Pelaku UMKM')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow">
        <div class="px-6 py-4 border-b">
            <h2 class="text-xl font-semibold text-gray-800">Form Tambah Pelaku UMKM</h2>
        </div>
        
        <form action="{{ route('admin.pelaku-umkm.store') }}" method="POST" class="p-6">
            @csrf
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="address" id="address" rows="3" 
                              class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 @error('address') border-red-500 @enderror">{{ old('address') }}</textarea>
                    @error('address')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
