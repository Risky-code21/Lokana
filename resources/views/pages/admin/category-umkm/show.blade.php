...
@section('title', 'Detail Kategori UMKM')
@section('page-title', 'Detail Kategori UMKM')
...
@section('content')
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex justify-between items-start mb-6">
            <div>
                ...
                <p class="text-gray-600 mt-1">Detail informasi kategori UMKM</p>
            </div>
            <div class="flex space-x-2">
                <a href="{{ route('admin.category-umkm.edit', $category) }}" 
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-lg shadow">
                    <div class="px-6 py-4 border-b flex justify-between items-center">
                        <h3 class="text-lg font-semibold text-gray-800">UMKM dalam Kategori</h3>
                        <span class="bg-blue-100 text-blue-800 text-xs font-medium px-3 py-1 rounded-full">
                            {{ $category->total_umkm }} Data
                        </span>
                    </div>
                    <div class="p-6">
                        @if($category->umkmProfiles->isEmpty())
                        <div class="text-center py-8 text-gray-500">
                            <i class="fas fa-store text-4xl mb-4"></i>
                            <p>Belum ada UMKM dalam kategori ini</p>
                        </div>
                        @else
                        <div class="space-y-4">
                            @foreach($category->umkmProfiles as $umkm)
                            <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100">
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-4">
                                        <p class="text-sm font-medium text-gray-900">{{ $umkm->name }}</p>
                                        <p class="text-xs text-gray-500">{{ $umkm->email }}</p>
                                    </div>
                                </div>
                            ...
                            @if($category->total_umkm > 10)
                            <div class="text-center pt-4">
                                <a href="#" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                                    Lihat semua {{ $category->total_umkm }} UMKM →
                                </a>
                            </div>
                            @endif
                        </div>
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    <div class="space-y-3">
                        <div class="flex justify-between">
                            <span class="text-gray-600">Total UMKM</span>
                            <span class="font-semibold">{{ $category->total_umkm }}</span>
                        </div>
                        <div class="flex justify-between">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                {{ $category->total_umkm > 0 ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                {{ $category->total_umkm > 0 ? 'Aktif' : 'Kosong' }}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    <div class="space-y-3">
                        ...
                        <form id="delete-form-{{ $category->id }}" 
                              action="{{ route('admin.category-umkm.destroy', $category) }}" 
                              method="POST" 
                              class="hidden">
                            @csrf
                            @method('DELETE')
                        ...
                        <button disabled
                                ...
                                title="Tidak dapat dihapus karena memiliki UMKM">
                            <i class="fas fa-trash mr-2"></i> Hapus Kategori
                        </button>
                        @endif
                        
                        <a href="{{ route('admin.category-umkm.create') }}" 
    ...
    function confirmDelete(id, name) {
        const deleteUrl = `{{ route('admin.category-umkm.destroy', ':id') }}`.replace(':id', id);
