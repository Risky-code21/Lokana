...
{{-- Container dari component checkbox --}}
<div class="relative inline-flex items-center gap-2">
    {{-- Input. menggunakan peer untuk membuat custom input yang lebih baik --}}
    {{-- Ketika input ini ditekan, maka dia akan mentrigger style elemen selanjutnya --}}
    <input type="checkbox" {{ $required ? 'required' : '' }} name="{{ $name }}" id="{{ $id }}"
        class="absolute z-20 inset-0 opacity-0 peer">

    {{-- View checkbox custom yang akan di trigger ketika input checked --}}
    <div
